...
**Updated:** 2026-04-14 — sticky sessions, TOR SOCKS identity reuse, unknown field warnings
**Branch:** main
...
- Supports HTTP proxy, CONNECT tunnels, WebSocket upgrades, and SOCKS5 server mode.
...
- Sticky sessions via `X-Proxy-Session` header pin traffic to one relay for 5 min of inactivity; automatic rebinding on unhealthy relay.
- TOR sticky sessions reuse one SOCKS auth identity per session key for stable circuit isolation.
- `POST /rotate` returns warnings for unknown fields in the request body.

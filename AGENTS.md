...
- `--relay-refresh-interval` (flag, in ms) or `RELAYRAD_RELAY_REFRESH_INTERVAL` (env) controls relay auto-refresh; default is 3,600,000 ms (60 min). Set to `0` to disable.
- The TUI only appears when stdin is a TTY and you passed none of: source flags (`--mullvad`, `--tor`, `--nordvpn`, `--github-lists`), config flags (`--port`, `-p`, `--socks5-port`, `--proxy-auth`, `--log-proxy-sqlite`), or `--no-tui`.
...
- Relay transport is protocol-driven: Mullvad and TOR use SOCKS5, NordVPN uses HTTP proxy over TLS, GitHub Lists uses SOCKS5 or plain HTTP CONNECT.
